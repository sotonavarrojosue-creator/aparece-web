// ShapeWaves — React Bits (JS + CSS) adaptado a ESM sin bundler.
// Dependencias vía import map: react, react-dom/client, vgpu.
...
export default function ShapeWaves({
  ...
  const settingsSignature = [
    shapes, cellSize, dotSize, color, hoverColor, backgroundColor,
    speed, scale, contrast, brightness, flow, direction, fade,
    interactive, splashRadius, splashStrength, glow, intro, introDuration, paused
  ].join('|');
  const maskSignature = [text, fontFamily, fontWeight, textSize].join('|');
  const replayRef = useRef(() => {});

  useEffect(() => { applySettingsRef.current(); }, [settingsSignature]);
  useEffect(() => { applyMaskRef.current(); }, [maskSignature]);
  useEffect(() => { if (introKey) replayRef.current(); }, [introKey]);
  ...
  return createElement(
    ...
    createElement('canvas', { ref: canvasRef, className: 'shape-waves__canvas' })
  );
}
